<?php

/* @var $this yii\web\View */
/* @var $orders app\models\Order */
/* @var $product app\models\OrderDetail */
use app\assets\AppAsset;

$this->title = '销售订单';

?>
<div class="row">
  <div class="col-lg-10 col-lg-offset-1">
    <?php foreach ($orders as $order): ?>
    <?php $products = $order['detail'] ?>
    <table class="table table-striped">
      <caption>订单编号: <?= $order['order']->order_id; ?></caption>
      <thead>
        <tr>
          <th>药品名称</th>
          <th>生产厂家</th>
          <th>数量</th>
          <th>所在仓库</th>
          <th>默认价格</th>
          <th>买家</th>
        </tr>
      </thead>
      <tbody>
        <?php foreach ($products as $product): ?>
          <tr>
            <td><?= $product->getProduct()->one()->product_name;  ?></td>
            <td><?= $product->getProduct()->one()->getManufacturer()->one()->manufacturer_name;  ?></td>
            <td><?= $product->product_num;  ?></td>
            <td><?= $product->getProduct()->one()->getRepository()->one()->repository_name; ?></td>
            <td><?= $product->getProduct()->one()->product_default_price;  ?></td>
            <td><?= $product->getOrder()->one()->getClient()->one()->client_name;  ?></td>
          </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
  <?php endforeach; ?>
    <span>
      <a href="?r=site/addproduct"><span class="btn btn-primary pull-right">增加销售订单</span></a>
    </span>
  </div>
</div>

<!--<?php AppAsset::addJsFile($this, '/js/site.js'); ?>-->
